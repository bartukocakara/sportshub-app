            ...
            <form action="{{ route('matches.update', $datas['match']->id) }}" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-5">
                        ...
                        <select name="team_status" id="update_match_status" class="form-select form-select-solid select2-status" required>
                            <option value="active" {{ old('match_status', $datas['match']->team_status) === 'active' ? 'selected' : '' }} data-html="<span class='badge badge-light-success'>{{ __('messages.active') }}</span>">{{ __('messages.active') }}</option>
                            <option value="inactive" {{ old('match_status', $datas['match']->team_status) === 'inactive' ? 'selected' : '' }} data-html="<span class='badge badge-light-warning'>{{ __('messages.inactive') }}</span>">{{ __('messages.inactive') }}</option>
                            <option value="banned" {{ old('match_status', $datas['match']->team_status) === 'banned' ? 'selected' : '' }} data-html="<span class='badge badge-light-danger'>{{ __('messages.banned') }}</span>">{{ __('messages.banned') }}</option>
                        </select>
                        @error('match_status')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-5">
                        ...
                        <select name="city_id" id="update_city_id" class="form-select form-select-solid @error('city_id') is-invalid @enderror" data-control="select2" data-dropdown-parent="#kt_modal_update_match_info" required>
                            @foreach($datas['cities'] as $city)
                                <option value="{{ $city->id }}" {{ old('city_id', $datas['match']->city_id) === $city->id ? 'selected' : '' }}>{{ $city->title }}</option>
                            @endforeach
                        </select>
                        @error('city_id')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                    ...
                    <div class="mb-5">
                        ...
                        <select name="sport_type_id" id="update_sport_type_id" class="form-select form-select-solid @error('sport_type_id') is-invalid @enderror" data-control="select2" data-dropdown-parent="#kt_modal_update_match_info" required>
                            @foreach($datas['sport_types'] as $type)
                                <option value="{{ $type->id }}" {{ old('sport_type_id', $datas['match']->sport_type_id) === $type->id ? 'selected' : '' }}>{{ $type->title }}</option>
                            @endforeach
                        </select>
                        @error('sport_type_id')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                    ...
                    <div class="mb-5">
                        ...
                        <div class="d-flex flex-wrap gap-5">
                            <div class="form-check form-check-custom form-check-solid">
                                <input class="form-check-input @error('gender') is-invalid @enderror" type="radio" name="gender" value="male" id="update_gender_male" {{ old('gender', $datas['match']->gender) == 'male' ? 'checked' : '' }}/>
                                <label class="form-check-label" for="update_gender_male">
                                    {{ __('messages.male') }}
                                </label>
                            </div>
                            <div class="form-check form-check-custom form-check-solid">
                                <input class="form-check-input @error('gender') is-invalid @enderror" type="radio" name="gender" value="female" id="update_gender_female" {{ old('gender', $datas['match']->gender) == 'female' ? 'checked' : '' }}/>
                                <label class="form-check-label" for="update_gender_female">
                                    {{ __('messages.female') }}
                                </label>
                            </div>
                            <div class="form-check form-check-custom form-check-solid">
                                <input class="form-check-input @error('gender') is-invalid @enderror" type="radio" name="gender" value="mixed" id="update_gender_mixed" {{ old('gender', $datas['match']->gender) == 'mixed' ? 'checked' : '' }}/>
