@extends('layouts.no-sidebar')
@section('title', __('messages.payment'))

@section('content')
<div class="app-main flex-column flex-row-fluid" id="kt_app_main">
    <div class="d-flex flex-column flex-column-fluid">
        <div id="kt_app_toolbar" class="app-toolbar pt-5">
            <div id="kt_app_toolbar_container" class="app-container container-fluid d-flex align-items-stretch">
                <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                    <div class="page-title d-flex flex-column m-3 mb-2 text-center w-100">
                        <h1 class="page-heading d-flex flex-column justify-content-center text-gray-900 fw-bolder fs-1 lh-0">
                            {{ __('messages.payment_failed') }}
                        </h1>
                    </div>
                </div>
            </div>
        </div>
        <div id="kt_app_content" class="app-content flex-column-fluid">
            <div class="app-container container-fluid d-flex justify-content-center align-items-center flex-column">
                <div class="card shadow-lg p-5 text-center w-50">
                    <div class="mb-4">
                        <i class="fas fa-times-circle text-danger display-1"></i>
                    </div>
                    <h2 class="text-danger fw-bold">{{ __('messages.payment_failed') }}</h2>
                    <p class="text-muted">{{ __('messages.payment_failed_message') }}</p>

                    <a href="{{ Auth::check() ? route('checkout.user.reservation') : route('checkout.guest.reservation') }}" class="btn btn-info mt-4">{{ __('messages.try_again') }}</a>
                    <a href="{{ route('home') }}" class="btn btn-primary mt-2">{{ __('messages.back_to_home') }}</a>
                </div>
            </div>
        </div>
    </div>
</div>
@endsection
