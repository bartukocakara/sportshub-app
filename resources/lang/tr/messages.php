<?php

return [
    'password' => 'Şifre',
    'forgot_password' => 'Şifremi unuttum',
    'reset_password' => 'Şifremi yenile',
    'forgot_password_link' => 'Şifremi unuttum linki',
    'warning_for_mail' => 'Bu maili neden aldığınızı bilmiyorsanız lütfen',
    'password_reset_info' => 'Parola sıfırlama sayfasına erişin ve parola kurtarma işlemini başlatmak için bu kodu girin. Kodu doğruladıktan sonra, hesabınız için yeni bir şifre belirlemeniz istenecektir.',
    'click_for_reset_password' => 'Şifrenizi yenilemek için tıklaynız.',
    'forget_password_time_limitation' => 'Bu kod 15 dakika için geçerlidir.',
    'navigate_in_app' => "Sportshub'ta gezin",
    'contact_with_us' => 'Bizimle iletişime geçin',
    'mode' => 'Mod',
    'PHYSICAL' => 'Fiziksel',
    'DESK' => 'Masa',
    'ONLINE' => 'Çevrimiçi',
    'TEAM' => 'Takım',
    'INDIVIDUAL' => 'Bireysel',
    'reset_password_confirmation' => "Şifreniz başarıyla sıfırlandı. Artık seçtiğiniz yeni şifreyi kullanarak hesabınıza giriş yapabilirsiniz. Bu değişikliği talep etmediyseniz veya herhangi bir sorunuz varsa lütfen destek ekibimizle iletişime geçmekten çekinmeyin.",
    'sportshub_support' => 'Sportshub Support',
    'navigate_in_sportshub' => 'Navigate in Sportshub',
    'welcome' => 'Hoşgeldiniz',
    'welcome_greeting' => 'Sportshub’a hoş geldiniz, :name!',
    'welcome_message' => 'Aramıza katıldığınız için çok heyecanlıyız! Sportshub’da sunduğumuz her şeyi keşfedin ve keyfini çıkarın.',
    'navigate_in_sportshub' => 'Sportshub’ı Keşfedin',
    'contact_support' => 'Destek ile İletişime Geçin',
    'match_invite_subject' => 'Maça Katılım Daveti',
    'match_invite_greeting' => 'Merhaba :name,',
    'match_invite_message' => 'Bir maça katılmanız için davet edildiniz! İşte maç detayları: :matchDetails.',
    'accept_invite' => 'Daveti Kabul Et',
    'match_join_request_subject' => 'Maç Katılma Talebi',
    'match_join_request_greeting' => 'Merhaba :name,',
    'match_join_request_message' => ':playerName maçınıza katılmak istiyor! İşte maç detayları: :matchDetails.',
    'approve_request' => 'Talebi Onayla',
    'decline_request' => 'Talebi Reddet',
    'reservation_created_subject' => 'Rezervasyon Oluşturuldu',
    'reservation_notification_owner' => ':courtName için yeni bir rezervasyon yapıldı!',
    'reservation_notification_player' => 'Merhaba :playerName, rezervasyonunuz başarıyla oluşturuldu!',
    'reservation_details' => 'Rezervasyon Detayları',
    'owner_contact_info' => ':playerName ile iletişime geçebilirsiniz.',
    'view_reservation' => 'Rezervasyonu Görüntüle',
    'forgot_password_subject' => 'Şifre Sıfırlama Talebi',
    'forgot_password_title' => 'Şifrenizi Mi Unuttunuz?',
    'forgot_password_greeting' => 'Merhaba :name,',
    'forgot_password_instruction' => 'Şifrenizi sıfırlamak için bir talep aldık. Şifrenizi sıfırlamak için aşağıdaki butona tıklayabilirsiniz:',
    'forgot_password_reset_button' => 'Şifrenizi Sıfırlayın',
    'forgot_password_footer' => 'Eğer şifre sıfırlama talebi göndermediyseniz, lütfen bu e-postayı görmezden gelin.',
    'court_created' => 'Saha oluşturuldu',
    'court_business_created' => 'Saha işletmesi oluşturuldu',
    'team_created' => 'Takım oluşturuldu',
    'match_created' => 'Maç oluşturuldu',
    'reservation_created' => 'Rezervasyon oluşturuldu',
    'reservation_confirmed' => 'Rezervasyon onaylandı',
    'status' => 'Durum',
    'address' => 'Adres',
    'sport_type' => 'Spor tipi',
    'title' => 'Başlık',
    'name' => 'İsim',
    'email' => 'Email adresi',
    'phone' => 'Telefon',
    'opening_time' => 'Açılış saati',
    'closing_time' => 'Kapanış saati',
    'coordinates' => 'Koordinatlar',
    'home' => 'Ana sayfa',
    'filter' => 'Filtrele',
    'make_reservation' => 'Rezerve et',
    'my_profile' => 'Profilim',
    'my_reservations' => 'Rezervasyonlarım',
    'logout' => 'Çıkış yap',
    'date' => 'Tarih',
    'date_filtering' => 'Tarih filtreleme',
    'price' => 'Fiyat',
    'price_filtering' => 'Fiyat filtreleme',
    'location_filtering' => 'Konum filtreleme',
    'sport_type' => 'Spor Tipi',
    'sport_type_filtering' => 'Spor Tipi filtreleme',
    'start_filter' => 'Filtreyi baslat',
    'back' => 'Geri dön',
    'please_wait' => 'Lütfen bekleyiniz',
    'court_business_detail' => 'Saha işletme detayları',
    'welcome_to_sportshub' => "Sportshub'a Hoşgeldiniz",
    'date_picker_from' => 'From Date',
    'date_picker_to' => 'To Date',
    'filter' => 'Filtrele',
    'empty_reservations' => 'Empty Reservations',
    'filtered_dates' => 'Filtered Results: From :from to :to',
    'no_empty_courts' => 'No empty courts available.',
    'filters' => 'Filtreler',
    'start_date' => 'Bu tarihten',
    'end_date' => 'End Date',
    'courts' => 'Sahalar',
    'announcements' => 'Duyurular',
    'reservations' => 'Rezervasyonlar',
    'location' => 'Location',
    'min_price' => 'Minimum fiyat',
    'max_price' => 'Maksimum fiyat',
    'start_date_and_end_date_are_required' => 'Başlangıç tarihi ve bitiş tarihi gereklidir.',
    'start_date_cannot_be_after_end_date' => 'Başlangıç tarihi, bitiş tarihinden sonra olamaz.',
    'start_date_cannot_be_in_the_past' => 'Başlangıç tarihi geçmişte olamaz.',
    'price_must_be_a_number' => 'Fiyat bir sayı olmalıdır.',
    'price_cannot_be_negative' => 'Fiyat negatif olamaz.',
    'min_price_cannot_be_greater_than_max_price' => 'Minimum fiyat, maksimum fiyattan büyük olamaz.',
    'time_range' => 'Zaman aralığı',
    'start_time' => 'Başlangıç zamanı',
    'end_time' => 'Bitiş zamanı',
    'from_hour' => 'Başlama saati',
    'to_hour' => 'Bitiş saati',
    'select_sport' => 'Spor seçiniz',
    'city' => 'İl',
    'district' => 'İlçe',
    'select_city' => 'İl seçiniz',
    'select_district' => 'İlçe seçiniz',
    'loading' => 'Yükleniyor...',
    'failed_to_load_districts' => 'İlçeler yüklenemedi.',
    'show_pricing_list' => 'Fiyatları göster',
    'monday'    => 'Pazartesi',
    'tuesday'   => 'Salı',
    'wednesday' => 'Çarşamba',
    'thursday'  => 'Perşembe',
    'friday'    => 'Cuma',
    'saturday'  => 'Cumartesi',
    'sunday'    => 'Pazar',
    'pricing_filter' => 'Fiyat filtreleme',
    'view_court_details' => 'Detay',
    'checkout_details' => 'Kontrol detayları',
    'customer_details' => 'Müşteri Detayları',
    'court_details' => 'Saha Detayları',
    'court_title' => 'Saha Başlığı',
    'court_type' => 'Saha Tipi',
    'court_location' => 'Saha Konumu',
    'court_price' => 'Saha Fiyatı',
    'customer_info' => 'Müşteri Bilgileri',
    'customer_name' => 'Müşteri Adı',
    'customer_email' => 'Müşteri E-posta',
    'customer_phone' => 'Müşteri Telefonu',
    'enter_customer_name' => 'Müşteri adını girin',
    'enter_customer_email' => 'Müşteri e-posta adresini girin',
    'enter_customer_phone' => 'Müşteri telefon numarasını girin',
    'reserved' => 'Rezerve edildi',
    'invalid_customer_name' => 'Lütfen 3 ile 255 karakter arasında geçerli bir ad girin.',
    'invalid_customer_email' => 'Lütfen geçerli bir e-posta adresi girin.',
    'invalid_customer_phone' => 'Lütfen 10 ile 12 rakam arasında geçerli bir telefon numarası girin.',
    'find_location' => 'Konum bul',
    'payment_details' => 'Ödeme detayları',
    'full_name' => 'Tam Ad',
    'first_name' => 'Ad',
    'last_name' => 'Soyad',
    'card_number' => 'Kart Numarası',
    'enter_card_number' => 'Kart Numarasını Girin',
    'expiry_date' => 'Son Kullanma Tarihi',
    'cvv' => 'Güvenlik Kodu (CVV)',
    'complete_payment' => 'Ödemeyi Tamamla',
    'make_payment' => 'Ödeme Yap',
    'payment_successfull' => 'Ödeme Başarılı',
    'payment_failed' => 'Ödeme Başarısız',
    'payment_success_message' => 'Ödemeniz başarıyla işlenmiştir.',
    'back_to_home' => 'Anasayfaya Dön',
    'court' => 'Saha',
    'sport_type_id' => 'Spor Türü',
    'city_id' => 'İl',
    'time' => 'Zaman',
    'try_again' => 'Yeniden deneyin',
    'login' => 'Giriş yap',
    'register' => 'Kayıt ol',
    'or' => 'Veya',
    'login_with_google' => 'Google ile giriş yap',
    'register_with_google' => 'Google ile kayıt ol',
    'confirm_password' => 'Şifreyi Doğrula',
    'birth_date' => 'Doğum Tarihi',
    'english' => 'İngilizce',
    'turkish' => 'Türkçe',
    'reviews' => 'Yorumlar',
    'business_info' => 'İşletme Bilgileri',
    'court_business' => 'Saha İşletmesi',
    'court_businesses' => 'Saha İşletmeleri',
    'language' => 'Dil',
    'dark' => 'Koyu',
    'light' => 'Açık',
    'view_details' => 'Detay görüntüle',
    'make_comment' => 'Yorum yap',
    'canceled_reservations' => 'Iptal edilen rezervasyonlar',
    'complete_reservations' => 'Tamamlanan rezervasyonlar',
    'cancel_reservation' => 'Rezervasyon iptal et',
    'cancel_reservation_confirmation' => 'Rezervasyon iptal etmek istediğinizden emin misiniz?',
    'cancel_reservation_confirmation_message' => 'Rezervasyon iptal edildi.',
    'completed_reservations' => 'Tamamlanan rezervasyonlar',
    'active_reservations' => 'Aktif rezervasyonlar',
    'payment_status_list' => [
        1 => 'Ödeme Bekleniyor',  // WAITING_FOR_PAYMENT
        2 => 'Ödeme Onaylandı',   // PAYMENT_APPROVED
        3 => 'Ödeme İptal Edildi', // PAYMENT_CANCELED
        4 => 'Ödeme İade Edildi'  // PAYMENT_REFUNDED
    ],
    'code' => 'Kod',
    'location_map' => 'Konum Haritası',
    'business_name' => 'Işletme Adı',
    'your_rating' => 'Puanınız',
    'commented' => 'Yorumlandı',
    'comment' => 'Yorum',
    'rating' => 'Puan',
    'show_comment' => 'Yorumu göster',
    'close' => 'Kapat',
    'create_comment' => 'Yorum yap',
    'submit' => 'Gönder',
    'apply' => 'Başlat',
    'select_month' => 'Ay Seç',
    'select_year' => 'Yıl Seç',
    'court_name' => 'Saha Adı',
    'account' => 'Hesap',
    'court_business_login' => 'Saha İşletmesi Girişi',
    'user_login' => 'Kullanıcı Girişi',
    'account_settings' => 'Hesap Ayarları',
    'personel_info' => 'Kişisel Bilgiler',
    'update_personal_details' => 'Kişisel Bilgileri Güncelle',
    'security' => 'Güvenlik',
    'manage_account_security_and_passwords' => 'Hesap güvenliğini ve şifrelerini yönet',
    'payments' => 'Ödemeler',
    'view_and_manage_payment_methods' => 'Ödeme yöntemlerini görüntüle ve yönet',
    'notifications' => 'Bildirimler',
    'adjust_notification_preferences' => 'Bildirim tercihlerini ayarla',
    'control_data_sharing_and_visibility' => 'Veri paylaşımını ve görünürlüğünü kontrol et',
    'professional_tools' => 'Profesyonel Araçlar',
    'access_tools_for_professionals' => 'Profesyoneller için araçlara eriş',
    'privacy' => 'Gizlilik',
    'price_range' => 'Fiyat Aralığı',
    'subscription_plans' => 'Abonelik Planları',
    'court_images' => 'Saha Resimleri',
    'create_announcement' => 'Duyuru Olustur',
    'matches' => 'Maçlar',
    'teams' => 'Takımlar',
    'players' => 'Oyuncular',
    'matches_list' => 'Maç Listesi',
    'teams_list' => 'Takım Listesi',
    'players_list' => 'Oyuncu Listesi',
    'match_details' => 'Maç Detayları',
    'team_details' => 'Takım Detayları',
    'player_details' => 'Oyuncu Detayları',
    'match' => 'Maç',
    'activities' => 'Aktiviteler',
    'no_address_available' => 'Adres bulunamadı',
    'create_team' => 'Takım oluştur',
    'filter_teams' => 'Takımları filtrele',
    'gender' => 'Cinsiyet',
    'male' => 'Erkek',
    'female' => 'Kadın',
    'age' => 'Yaş',
    'team_name' => 'Takım Adı',
    'team_image' => 'Takım Resmi',
    'team_players' => 'Takım Oyuncuları',
    'team_matches' => 'Takım Maçları',
    'team_announcements' => 'Takım Duyuruları',
    'team_activities' => 'Takım Etkinlikleri',
    'max_player' => 'Maksimum Oyuncu',
    'min_player' => 'Minimum Oyuncu',
    'cancel' => 'İptal',
    'save' => 'Kaydet',
    'filter_matches' => 'Maçları Filtrele',
    'apply_filters' => 'Filtreleri Uygula',
    'reset_filters' => 'Filtreleri Sıfırla',
    'search' => 'Ara',
    'no_results_found' => 'Sonuç bulunamadı',
    'search_team_title' => 'Takım Başlığı Ara',
    'clear_filter' => 'Filtre Temizle',
    'new_player' => 'Yeni Oyuncu',
    'search_match_title' => 'Maç Başlığı Ara',
    'delete_team' => 'Takımı Sil',
    'actions' => 'İşlemler',
    'edit' => 'Düzenle',
    'details' => 'Detaylar',
    'dashboard' => 'Göstergeler',
    'court_business_create' => 'Saha İşletmesi Oluştur',
    'court_business_update' => 'Saha İşletmesi Güncelle',
    'create' => 'Oluştur',
    'no_players_selected' => 'Oyuncu seçilmedi',
    'selected_players' => 'Seçilen Oyuncular',
    'remove_all' => 'Hepsini Kaldır',
    'messages' => 'Mesajlar',
    'created_at' => 'Olusturulma Tarihi',
    'avatar' => 'Avatar',
    'court_list' => 'Saha Listesi',
    'create_court' => 'Saha Olustur',
    'update_court' => 'Saha Guncelle',
    'general_information' => 'Genel Bilgiler',
    'select_sport_type' => 'Spor Türü Seçiniz',
    'remove_box' => 'Kutu Kaldır',
    'add_image' => 'Görsel Ekle',
    'save_changes' => 'Kaydet',
    'court_created_successfully' => 'Saha başarıyla oluşturuldu',
    'court_updated_successfully' => 'Saha başarıyla güncellendi',
    'court_deleted_successfully' => 'Saha başarıyla silindi',
    'longitude' => 'Boylam',
    'latitude' => 'Enlem',
    'map_location' => 'Harita Konumu',
    'street_name' => 'Sokak Adı',
    'address_detail' => 'Adres Detayı',
    'building_number' => 'Bina Numarası',
    'neighborhood' => 'Mahalle',
    'failed_to_update_court' => 'Kort güncellenirken bir hata oluştu. Lütfen tekrar deneyiniz.',
    'confirm_delete' => 'Bu öğeyi silmek istediğinizden emin misiniz?',
    'confirm_delete_title' => 'Silme Onayı',
    'delete' => 'Sil',
    'view' => 'Görüntüle',
    'no_data' => 'Veri bulunamadı',
    'enter_team_name' => 'Takım adını girin',
    'min_players' => 'Minimum Oyuncu',
    'max_players' => 'Maksimum Oyuncu',
    'mixed' => 'Diğer',
    'e_g_20' => 'En fazla 20',
    'e_g_4' => 'En az 4',
    'edit_team' => 'Takımı Düzenle',
    'team_updated_successfully' => 'Takım başarıyla güncellendi',
    'team_deleted_successfully' => 'Takım başarıyla silindi',
    'team_created_successfully' => 'Takım başarıyla oluşturuldu',
    'failed_to_create_team' => 'Takım oluşturulamadı. Lütfen tekrar deneyiniz.',
    'failed_to_delete_team' => 'Takım silinemedi. Lütfen tekrar deneyiniz.',
    'active' => 'Aktif',
    'inactive' => 'Pasif',
    'banned' => 'Askıya Alındı',
    'blocked' => 'Engellendi',
    'suspended' => 'Askıya Alındı',
    'expired' => 'Sonlandı',
    'cancelled' => 'İptal Edildi',
    'completed' => 'Tamamlandı',
    'pending' => 'Bekliyor',
    'approved' => 'Onaylandı',
    'rejected' => 'Reddedildi',
    'refunded' => 'İade Edildi',
    'create_match' => 'Maç oluştur',
    'match_created_successfully' => 'Maç başarıyla oluşturuldu',
    'match_updated_successfully' => 'Maç başarıyla güncellendi',
    'match_deleted_successfully' => 'Maç başarıyla silindi',
    'failed_to_create_match' => 'Maç oluşturulamadı. Lütfen tekrar deneyiniz.',
    'failed_to_update_match' => 'Maç güncellenirken bir hata oluştu. Lütfen tekrar deneyiniz.',
    'failed_to_delete_match' => 'Maç silinemedi. Lütfen tekrar deneyiniz.',
    'profile' => 'Profil',
    'whats_on_your_mind' => "Ne düşünüyorsun :name?",
    'filter_announcements' => 'Duyuruları Filtrele',
    'search_announcement_title' => 'Duyuru Başlığı Ara',
    'select_gender' => 'Cinsiyet Seçiniz',
    'other' => 'Diğer',
    'filter_users' => 'Kullanıcıları Filtrele',
    'search_full_name' => 'Tam Adı Ara',
    'select_location' => 'Konum Seçiniz',
    'activity_list' => 'Aktiviteler',
    'announcement_list' => 'Duyurular',
    'user_list' => 'Kullanıcılar',
    'announcement_details' => 'Duyuru Detayları',
    'announcement_created_successfully' => 'Duyuru başarıyla oluşturuldu',
    'announcement_updated_successfully' => 'Duyuru başarıyla güncellendi',
    'announcement_deleted_successfully' => 'Duyuru başarıyla silindi',
    'filter_activities' => 'Aktiviteleri Filtrele',
    'match.created' => ':user bir maçı oluşturdu',
    'match.joined' => ':user bir maça katıldı',
    'match.updated' => ':user bir maçı güncelledi',
    'match.canceled' => ':user bir maçı iptal etti',
    'team.created' => ':user bir takım oluşturdu',
    'team.joined' => ':user bir takıma katıldı',
    'team.updated' => ':user bir takımı güncelledi',
    'court.created' => ':user bir saha oluşturdu',
    'requested_players' => 'Onay Bekleyen Oyuncular',
    'add_new_players' => 'Yeni Oyuncu Ekle',
    'join' => 'Katıl',
    'joined' => 'Katıldı',
    'join_request_sent' => 'Katılım isteği gönderildi',
    'join_request_accepted' => 'Katılım isteği kabul edildi',
    'join_request_rejected' => 'Katılım isteği reddedildi',
    'following' => 'Takip ediyorsun',
    'follow' => 'Takip et',
    'unfollow' => 'Takipten çık',
    'member' => 'Uye',
    'waiting_for_approval' => 'Onay Bekliyor',
    'leader' => 'Yönetici',
    'delete_team_warning' => 'Bu takımı silmek istediginizden emin misiniz?',
    'delete_match_warning' => 'Bu maçı silmek istediginizden emin misiniz?',
    'delete_announcement_warning' => 'Bu duyuruı silmek istediginizden emin misiniz?',
    'delete_court_warning' => 'Bu sahaı silmek istediginizden emin misiniz?',
    'delete_confirmation' => 'Onayla',
    'ok' => 'Tamam',
    'team_deleted_title' => 'Takım Silindi!',
    'team_deleted_body_line_1' => 'Takım artık aramızda değil...',
    'team_deleted_body_line_2' => 'Bu eylem geri alınamaz. Tüm ilişkili bilgiler kalıcı olarak kaldırıldı.',
    'unexpected_error' => 'Bir şeyler ters gitti!',
    'contact_support_prompt' => 'Lütfen destek ile iletişime geçiniz. Bu hata uygulama yöneticisine bildirilecektir.',
    'contact_us' => 'İletişime Geç',
    'team_leaders' => 'Takım Yöneticileri',
    'quit_team' => 'Takımdan Ayrıl',
    'quit_membership' => 'Üyelikten Ayrıl',
    'quit' => 'Ayrıl',
    'quit_membership_warning' => 'Takımdan ayrılmak istediğinizden emin misiniz? Bu işlem geri alınamaz.',
    'player_deleted_successfully' => 'Takımdan başarıyla ayrıldınız.',
    'failed_to_delete_player' => 'Takımdan ayrılırken hata oluştu. Lütfen tekrar deneyin veya destek ile iletişime geçin.',
    'quit_team_emotional_warning' => 'Bu işlem geri alınamaz.Bütün iliskileriniz kalıcı olarak silinecektir.',
    'delete_emotional_warning' => 'Bu işlem geri alınamaz.Bütün verileriniz kalıcı olarak silinecektir.',
    'quit_team_successfull_title' => 'Takımdan basarıyla ayrıldınız!',
    'quit_team_successfull_body' => 'Takımdan basarıyla ayrıldınız.',
    'see_you_again' => 'Yeniden görüşmek üzere!',
    'join_team_title' => 'Takıma Katılmak İstediğine Emin misin?',
    'join_team_confirmation' => 'Bu takıma katılmak istediğini onaylıyor musun?',
    'join_team_emotional_warning' => 'Takım arkadaşların seni bekliyor olacak 🤝. Unutma, birlikte daha güçlüsünüz!',
    'team_join_request_sent_successfully' => 'Takıma katılım istegi gönderildi.',
    'team_join_request_sent_successfully_title' => 'Takıma katılım istegi gönderildi',
    'team_join_request_sent_successfully_body' => 'Takıma katılım istegi gönderildi.',
    'team_join_request_sent_failed' => 'Takıma katılım istegi gönderilemedi. Lütfen tekrar deneyin veya destek ile iletişime geçin.',
    'team_join_request_sent_failed_title' => 'Takıma katılım istegi gönderilemedi',
    'team_join_request_sent_failed_body' => 'Takıma katılım istegi gönderilemedi. Lütfen tekrar deneyin veya destek ile iletişime geçin.',
    'optional_join_message' => 'Opsiyonel Katılım Mesajı',
    'write_a_message_to_join' => 'Bir mesaj yazın(İletişim bilgilerinizi iletebilirsiniz email adresi veya telefon numarası gibi)',
    'request_cancelled_title' => 'Istek iptal edildi',
    'request_cancelled_small' => 'Takım arkadaşların seni bekliyor olacak 🤝. Unutma, birlikte daha güçlüsünüz!',
    'cancel_request' => 'İsteği İptal Et',
    'cancel_request_title' => 'İsteği Geri Çekmek İstiyor musun?',
    'cancel_request_message' => 'Gönderdiğin katılım isteğini iptal edeceksin.',
    'cancel_request_emotional_warning' => 'İstersen tekrar başvurabilirsin.',
    'request_cancelled_body' => 'Katılma isteğin başarıyla geri çekildi.',
    'invited_players' => 'Davet Edilen Oyuncular',
    'reject_invitation' => 'Daveti Reddet',
    'accept_invitation_message' => 'Davetini Kabul Etmek İstiyor musun?',
    'accept_invitation_emotional_warning' => 'Yeni bir istek gönderebilirsin.',
    'invite_rejection_failed' => 'Davet reddedilemedi. Lütfen tekrar deneyin veya destek ile iletişime gelecek.',
    'invite_rejected_body' => 'Davet reddedildi.',
    'reject_invitation_title' => 'Davetini Reddet',
    'request_rejection_failed' => 'Davet reddedilemedi. Lütfen tekrar deneyin veya destek ile iletişime gelecek.',
    'reject_invitation_message' => 'Davetini Reddetmek İstiyor musun?',
    'reject_invitation_emotional_warning' => 'Yeni bir istek gönderebilirsin.',
    'accept_invitation' => 'Daveti Kabul Et',
    'accept_invitation_body' => 'İstek basarıyla kabul edildi.',
    'reject_invitation_body' => 'Davet reddedildi.',
    'invite_rejected_title' => 'Davet Reddedildi',
    'invite_rejected_small' => 'Yeni bir istek gönderebilirsin.',
    'success' => 'Basarılı',
    'failed' => 'Basarisiz',
    'error' => 'Hata',
    'follow_success' => 'Takımı takip ediyorsunuz',
    'unfollow_success' => 'Takımı takipten çıktınız',
    'current_players' => 'Mevcut Oyuncular',
    'go_to_announcements' => 'Duyurulara Git',
    'show_more' => 'Daha Fazla Görüntüle',
    'post' => 'Gönder',
    'write_your_announcement' => 'Duyurunuzu yazınız.',
    'announcement_created_successfully_prompt' => 'Duyurunuz oluşturuldu.',
    'type' => 'Tipi',
    'participant' => 'Katılımcı',
    'inviter' => 'Davet Eden',
    'participants' => 'Katılımcılar',
    'individual' => 'Bireysel',
    'team' => 'Takım',
    'select_type' => 'Tipi Seçiniz',
    'message' => 'Mesaj',
    'subject' => 'Konu',
    'delete_announcement_message' => 'Duyuruyu silmek istediğinize emin misiniz?',
    'delete_announcement_title' => 'Duyuru Sil',
    'user' => 'Kullanıcı',
    'suggestions_for_you' => 'Senin icin öneriler',
    'view_all' => 'Hepsini Görüntüle',
    'no_suggestions' => 'Hicbir ozet yok',
    'update_announcement_title' => 'Duyuru Güncelle',
    'update_announcement_message' => 'Duyuruyu güncellemek istediğinize emin misiniz?',
    'announcement_updated_successfully_prompt' => 'Duyurunuz güncellendi.',
    'delete_announcement_emotional_warning' => 'Bu islem gercekten yapılacaksa, ilgili bilgiler kalıcı olarak silinecektir.',
    'update_announcement_emotional_warning' => 'Bu islem gercekten yapılacaksa, ilgili bilgiler kalıcı olarak silinecektir.',
    'announcement_deleted_successfully_prompt' => 'Duyurunuz silindi.',
    'select_players' => 'Oyuncuları Seçiniz',
    'fill_team_details' => 'Takım Detaylarını Doldurunuz',
    'team_creation_start_prompt' => 'Takım oluşturma işlemini başlatmak için lütfen spor tipini seçiniz.',
    'continue' => 'Devam Et',
    'select_player' => 'Oyuncu Seçiniz',
    'confirm_details' => 'Detayları Onayla',
    'allow_following' => 'Takiplere izin ver',
    'allow_others_to_follow_your_team' => 'Takımınızı takip etme izni veriniz.',
    'followable_status' => 'Takip Edilebilir',
    'review_and_confirm' => 'Detayları gözden geçir ve onayla',
    'gender_preference' => 'Cinsiyet Tercihi',
    'review_your_team_details' => 'Takımınızı detaylarına gözden gecirin.',
    'please_review_the_details_below_before_creating_your_team' => 'Lütfen takımınızı oluşturmak için detayları gözden geciriniz.',
    'yes' => 'Evet',
    'no' => 'Hayır',
    'confirm_team_details' => 'Takım Detaylarını Onayla',
    'team_invite_request_title' => ':title takımına davet edildiniz.',
    'team_invite_email_body' => ':inviter_name seni :team_title takımına katılman için davet etti.',
    'no_more_results' => 'Başka sonuç yok...🌵',
    'team_players_min_count_error' => 'Takım oyuncu minimum sayısı hatası',
    'add_more_players_to_invite_list' => 'Davet listesine daha fazla oyuncu ekle',
    'team_players_max_count_error' => 'Takım oyuncu maksimum sayısı hatası',
    'remove_some_players_from_invite_list' => 'Davet listesinden bazı oyuncuları kaldırın',
    'warning' => 'Uyarı',
    'access_denied' => 'Erisim Engellendi',
    'access_denied_prompt' => 'Bu sayfaya erisim yetkiniz bulunmamaktadır.',
    'delete_request_player_team_message' => 'Daveti silmek istediğinize emin misiniz?',
    'request_player_team_deleted_successfully' => 'Davet basarıyla silindi.',
    'request_player_team_not_found' => 'Katılım istegi bulunamadı.',
    'request_player_team_accepted_successfully' => 'Katılım istegi kabul edildi.',
    'request_player_team_rejected_successfully' => 'Katılım istegi reddedildi.',
    'accept' => 'Kabul Et',
    'reject' => 'Reddet',
    'accept_confirmation' => 'Katılım istegi kabul etmek istediğinize emin misiniz?',
    'reject_confirmation' => 'Katılım istegi reddetmek istediğinize emin misiniz?',
    'accept_request_player_team_message' => ':full_name adlı kişinin katılım isteğini kabul etmek istiyor musunuz?',
    'error_occurred_while_accepting' => 'Katılım istegi kabul edilirken bir hata oluştu.',
    'error_occurred_while_rejecting' => 'Katılım istegi reddedilirken bir hata oluştu.',
    'reject_request_player_team_message' => ':full_name adlı kişinin katılım isteğini reddetmek istiyor musunuz?',
    'delete_player_team_message' => 'Oyuncuyu çıkarmak istediğinize emin misiniz?',
    'remove' => 'Çıkar',
    'player_team_deleted_successfully' => 'Oyuncu basarıyla çıkardı.',
    'you_are_awesome' => 'Harikasın!',
    'ready_to_invite_player' => 'Bu oyuncuyu davet etmeye hazır mısın?',
    'this_will_make_the_team_even_better' => 'Takımınız daha da güçlenecek!',
    'invite_emotional_warning' => 'Oyuncuyu dahil etmek üzeresiniz.Bu işlem geri alınabilir.',
    'send_invitation' => 'Davet Gönder',
    'not_now' => 'Şimdi Değil',
    'invite' => 'Davet Et',
    'invite_player_team_message' => 'Bu oyuncuyu takımına davet etmek istediğine emin misin?',
    'invite_confirmation' => 'Oyuncuyu Davet Et',
    'enter_title' => 'Bir baslık yazın',
    'player_invited_successfully' => 'Oyuncu basarıyla davet edildi.',
    'welcome_to_sportshub' => '👋 Sportshub\'a Hoş Geldin! Sporun Yeni Merkezi Burada! 🚀',
    'discover_intro' => 'Yeni oyuncular keşfet, hayalindeki takımı bul, en iyi sahaları rezerve et ve maçları kaçırma! Oyunun seviyesini yükseltmeye hazır ol. 🥇',
    'get_started' => 'Hemen Başla! 🎉',
    'explore_features' => 'Özellikleri Keşfet 👇',
    'whats_inside' => 'Sportshub\'ta Neler Var? ✨',

    'find_new_players' => 'Yeni Oyuncular Bul',
    'find_new_players_desc' => 'Bölgenizdeki sporcularla bağlantı kurun, ağınızı genişletin ve her spor için mükemmel takım arkadaşlarını bulun. Beceriye, konuma ve oyun tercihine göre arayın! 🤝',

    'join_create_teams' => 'Takımlara Katıl / Takım Kur',
    'join_create_teams_desc' => 'Bir takım mı arıyorsunuz yoksa kendi ekibinizi mi kuruyorsunuz? Sportshub ile takımları kolayca yönetin, antrenmanları organize edin ve birlikte yarışın. 🏅',

    'book_courts' => 'Saha ve Mekan Rezerve Et',
    'book_courts_desc' => 'Yakındaki spor sahalarını ve mekanlarını kolayca bulun ve anında rezerve edin. Müsaitlik durumunu kontrol edin ve oyuna hazır olun! 🗓️',

    'organize_matches' => 'Maçları Düzenle ve Bul',
    'organize_matches_desc' => 'Gündelik oyunlar veya rekabetçi maçlar kurun. Yaklaşan maçları görüntüleyin, diğer takımlara meydan okuyun ve skorlarınızı takip edin. 🔥',

    'announcements_short' => 'Duyurularla Güncel Kal',
    'announcements_desc' => 'Maç değişiklikleri, takım güncellemeleri, lig haberleri ve özel etkinlikler için anlık bildirimler alın. Hiçbir gelişmeyi kaçırmayın! 🔔',

    'track_progress' => 'Gelişimini Takip Et',
    'track_progress_desc' => 'Performansınızı izleyin, maç sonuçlarını takip edin ve istatistiklerinizin zamanla nasıl geliştiğini görün. Başarılarınızı kutlayın, hedeflerinize odaklanın! 🚀',

    'cta_title' => 'Oyuna Katılmaya Hazır mısın? 🏃‍♀️💨',
    'cta_desc' => 'Bugün Sportshub topluluğuna katıl ve spor deneyimini bir üst seviyeye taşı!',
    'sign_up_now' => 'Hemen Kaydol! 🥳',
    'find_match' => 'Maç Bul',
    'find_team' => 'Takım Bul',
    'find_court' => 'Saha Bul',
    'find_player' => 'Oyuncu Bul',
    'be_informed_about_announcements' => 'Duyurulardan Haberdar Ol',
    'welcome_to_sportshub'=> 'Sportshuba hoş geldiniz! 🚀',
    'min_team' => 'Minimum Takım',
    'max_team' => 'Maksimum Takım',
    'quit_match' => 'Maçtan Ayrıl',
    'participant' => 'Katılımcı',
    'unknown' => 'Bilinmeyen',
    'me_match_team_player_quit_successfully' => 'Maçtan başarıyla ayrıldınız.',
    'match_team_player_delete_successfully' => 'Oyuncu maçtan başarıyla çıkarıldı.',
    'match_owner' => 'Maç Sahibi',
    'join_match_title' => 'Maça Katıl',
    'join_match_confirmation' => 'Maça katılmak istediğinize emin misiniz?',
    'join_match_emotional_warning' => 'Takım arkadaşların seni bekliyor olacak 🤝. Unutma, birlikte daha güçlüsünüz.',
    'match_join_request_sent_successfully' => 'Maç katılım isteği gönderildi.',
    'match_join_request_sent_failed' => 'Maç katılım isteği gönderilemedi. Lütfen tekrar deneyin veya destek ile iletişime geçin.',
    'none' => 'Yok',
    'canceled' => 'İptal Edildi',
    'confirmed' => 'Onaylandı',
    'accepted' => 'Kabul Edildi',
    'match_deleted_body_line_1' => 'Maç basarıyla silindi.',
    'you_can_drag_and_drop_players_between_teams_to_reorganize' => 'Takımları yeniden döndürmek için oyuncuları takımlar arasında tıklayın ve bırakın.',
    'drag_and_drop_players' => 'Oyuncuları takımlar arasında tıklayın ve bırakın.',
    'no_teams_found' => 'Takım bulunamadı.',
    'team_list' => 'Takım Listesi',
    'create_match_team' => 'Maç Takımı Oluştur',
    'team_title' => 'Takım Başlığı',
    'match_team_created_successfully' => 'Maç takımı başarıyla oluşturuldu',
    'save_team_changes' => 'Değişiklikleri Kaydet',
    'team_changes_saved' => 'Oyuncu değişiklikleri başarıyla kaydedildi.',
    'error_occurred' => 'Bir hata oluştu.',
    'edit_match' => 'Maçı Düzenle',
    'play_date' => 'Oynama Tarihi',
    'basic_info' => 'Temel Bilgiler',
    'description' => 'Açıklama',
    'match_type_and_gender' => 'Maç Türü ve Cinsiyet',
    'player_limits' => 'Oyuncu Sınırları',
    'schedule' => 'Program',
    'remove_player' => 'Oyuncuyu Kaldır',
    'delete_player_title' => 'Oyuncuyu Sil',
    'delete_player_message' => 'Bu oyuncuyu takımdan çıkarmak istediğinize emin misiniz?',
    'delete_player_emotion' => 'Bu işlem geri alınamaz. Oyuncu ve tüm ilişkili bilgileri kalıcı olarak kaldırılacaktır.',
    'open_delete_mode' => 'Silme Modunu Aç',
    'close_delete_mode' => 'Silme Modunu Kapat',
    'match_team_updated_successfully' => 'Maç takımı başarıyla güncellendi',
    'delete_team_title' => 'Takımı Sil',
    'delete_team_message' => 'Bu takımı silmek istediğinize emin misiniz?',
    'delete_team_emotion' => 'Bu işlem geri alınamaz. Takım ve tüm ilişkili bilgileri kalıcı olarak kaldırılacaktır.',
    'match_team_not_found' => 'Maç takımı bulunamadı.',
    'match_team_deleted_successfully' => 'Maç takımı başarıyla silindi',
    'remove_team' => 'Takımı Kaldır',
    'no_requested_players_found' => 'Onay bekleyen oyuncu bulunamadı',
    'no_invited_players_found' => 'Davet edilen oyuncu bulunamadı',
    'invite_player_message' => 'Bu oyuncuyu davet etmek istediğinize emin misiniz?',
];
